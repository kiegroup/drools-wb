...
@Dependent
public class AllScenarioSimulationAssetsDropdownProviderImpl implements ScenarioSimulationAssetsDropdownProvider {
    ...
    public void getItems(Consumer<List<KieAssetsDropdownItem>> assetListConsumer) {
        List<KieAssetsDropdownItem> toAccept = ScesimFilesProvider.fileMap.keySet().stream()
                .filter(key -> !key.toLowerCase().startsWith("new"))
