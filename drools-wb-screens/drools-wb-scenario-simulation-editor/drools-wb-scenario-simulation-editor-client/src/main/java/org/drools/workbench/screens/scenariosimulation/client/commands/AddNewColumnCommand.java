...
public class AddNewColumnCommand implements Command {
    ...
    public AddNewColumnCommand(ScenarioGridPanel scenarioGridPanel, ScenarioGridLayer gridLayer) {
    ...
    @Override
    public void execute() {
        GWT.log("AddNewColumnCommand execute");
        GridData model = scenarioGrid.getModel();
        model.appendColumn(getScenarioGridColumn("", scenarioGridPanel, gridLayer));
        int columnIndex = model.getColumnCount() -1;
        IntStream.range(0, model.getRowCount())
                .forEach(rowIndex -> model.setCell(rowIndex, columnIndex, () -> new ScenarioGridCell(new ScenarioGridCellValue(""))));
    }
}
