...
public class ListGroupItemPresenter implements ListGroupItemView.Presenter {
    ...
    protected void populateListGroupItemView(ListGroupItemView toPopulate, String parentPath, String factName, FactModelTree factModelTree) {
        ...
        factModelTree.getSimpleProperties().entrySet().stream().sorted(Map.Entry.comparingByKey()).forEach(
                (entry -> toPopulate.addFactField(fieldItemPresenter.getLIElement(fullPath, factName, entry.getKey(), entry.getValue()))));
        factModelTree.getExpandableProperties().entrySet().stream().sorted(Map.Entry.comparingByValue()).forEach(
                (entry -> toPopulate.addExpandableFactField(getDivElement(fullPath, entry.getKey(), entry.getValue()))));
    }
