...
public class FieldItemPresenter implements FieldItemView.Presenter {
    ...
    @Override
    public LIElement getLIElement(String parentPath, String factName, String rootFactName, String rootClassName, String fieldName, String className, String classTypeName) {
        String key = parentPath  + "." + fieldName;
        if (!fieldItemMap.containsKey(key)) {
            FieldItemView fieldItemView = viewsProvider.getFieldItemView();
            fieldItemView.setFieldData(parentPath, factName, rootFactName, rootClassName, fieldName, className, classTypeName);
