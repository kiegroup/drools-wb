...
public class EnableTestToolsInstanceCommandTest extends AbstractScenarioSimulationCommandTest {

    @Before
    public void setup() {
        super.setup();
        command = spy(new EnableTestToolsInstanceCommand());
    ...
    public void executeWithFactName() {
        ...
        verify(scenarioSimulationEditorPresenterMock, times(1)).reloadTestTools(false);
        verify(testToolsPresenterMock, times(1)).onEnableEditorTab(eq(FACT_NAME));
    ...
    public void executeWithoutFactName() {
        ...
        verify(scenarioSimulationEditorPresenterMock, times(1)).expandToolsDock();
        verify(scenarioSimulationEditorPresenterMock, times(1)).reloadTestTools(false);
        verify(testToolsPresenterMock, never()).onEnableEditorTab(anyString());
