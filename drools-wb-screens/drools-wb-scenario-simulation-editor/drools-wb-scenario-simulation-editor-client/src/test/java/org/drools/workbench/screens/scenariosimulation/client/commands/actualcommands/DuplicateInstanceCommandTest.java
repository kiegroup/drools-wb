...
public class DuplicateInstanceCommandTest extends AbstractScenarioGridCommandTest {
    ...
    public void setup() {
        super.setup();
        command = spy(new DuplicateInstanceCommand(GridWidget.SIMULATION) {
            @Override
    ...
    public void executeIfSelectedColumn_WithInstanceAndPropertyAndThreeColumns() {
        ...
        verify((DuplicateInstanceCommand) command, times(1)).setPropertyHeader(eq(scenarioSimulationContextLocal), eq(gridColumnMock), eq(expectedPropertyNameElementsSecond), eq(VALUE_CLASS_NAME), eq(GRID_PROPERTY_TITLE + "_2"));
