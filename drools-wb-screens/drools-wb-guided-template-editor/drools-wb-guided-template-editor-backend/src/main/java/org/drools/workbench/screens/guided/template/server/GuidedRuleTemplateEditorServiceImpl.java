...
public class GuidedRuleTemplateEditorServiceImpl
    ...
    public List<ValidationMessage> validate( final Path path,
                                             final TemplateModel model ) {
        try {
            final List<ValidationMessage> messages = validateTemplateVariables( path, model );
            final String content = RuleTemplateModelXMLPersistenceImpl.getInstance().marshal( model );

            messages.addAll( genericValidator.validate( path, content ) );
