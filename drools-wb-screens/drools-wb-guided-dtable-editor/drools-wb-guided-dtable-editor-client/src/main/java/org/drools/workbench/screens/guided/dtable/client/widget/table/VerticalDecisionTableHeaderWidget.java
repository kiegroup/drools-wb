...
import org.drools.workbench.screens.guided.dtable.client.widget.table.header.HeaderSorter;
import org.drools.workbench.screens.guided.dtable.client.widget.table.header.HeaderSplitter;
...
public class VerticalDecisionTableHeaderWidget extends AbstractDecoratedGridHeaderWidget<GuidedDecisionTable52, BaseColumn> {
    ...
    public void redraw() {
        ...
        for ( int iCol = 0; iCol < sortableColumns.size(); iCol++ ) {
            ...
            if ( col.isVisible() ) {
                ...
                if ( modelCol instanceof ConditionCol52 ) {
                    ...
                    widget.addVisibleConditionColumn( col );
                }
                if ( modelCol instanceof ActionCol52 ) {
                    ...
                    widget.addVisibleActionColumn( col );
                }
                iColumnCount++;
            }
        }
        ...
        // Draw rows
        for ( int iRow = 0; iRow < widget.getHeaderRowsCount(); iRow++ ) {
    ...
    private void setCursorType( Cursor cursor ) {
        for ( int iRow = 0; iRow < widget.getHeaderRowsCount(); iRow++ ) {
            TableRowElement tre = widget.getHeaderRow( iRow );
    ...
    protected Widget getHeaderWidget() {
        if ( this.widget == null ) {
            this.widget = new HeaderWidget(new HeaderSplitter(resources, VerticalDecisionTableHeaderWidget.this));
        }
        return widget;
    }
    ...
    protected ResizerInformation getResizerInformation( int mx ) {
        ...
        for ( int iCol = 0; iCol < widget.getHeaderRow( 4 ).getChildCount(); iCol++ ) {
            TableCellElement tce = widget.getHeaderRow( 4 ).getChild( iCol ).<TableCellElement>cast();
            int cx = tce.getAbsoluteRight();
            if ( Math.abs( mx - cx ) <= 5 ) {
                isPrimed = true;
                resizerInfo.setResizePrimed( isPrimed );
                resizerInfo.setResizeColumn( widget.getVisibleColumn( iCol ) );
    ...
    protected void resizeColumn( DynamicColumn<BaseColumn> resizeColumn,
        ...
        int resizeColumnIndex = widget.getIndexOfVisibleColumn( resizeColumn );

        // Row 0 (General\Fact Type)
        // General row, all visible cells included
        int iRow0ColColumn = 0;
        for ( int iCol = 0; iCol < widget.getVisibleColumnsCount(); iCol++ ) {
            DynamicColumn<BaseColumn> col = widget.getVisibleColumn( iCol );
            BaseColumn modelCol = col.getModelColumn();
            ...
            if ( modelCol instanceof BRLVariableColumn ) {
                ...
                while ( iCol + colSpan < widget.getVisibleColumnsCount() ) {
                    DynamicColumn<BaseColumn> mergeCol = widget.getVisibleColumn( iCol + colSpan );
            ...
            // Resize cell
            tce = widget.getHeaderRow( 0 ).getChild( iRow0ColColumn ).<TableCellElement>cast();
        ...
        // Row 4 (Sorters)
        tce = widget.getHeaderRow( 4 ).getChild( resizeColumnIndex ).<TableCellElement>cast();
        div = tce.getFirstChild().<DivElement>cast();
        ...
        if ( multiRowColumnOffset != -1 ) {
            ...
            if ( colOffsetIndex >= 0 ) {
                DynamicColumn<BaseColumn> col = widget.getVisibleColumn( resizeColumnIndex );
                tce = widget.getHeaderRow( 3 ).getChild( colOffsetIndex ).<TableCellElement>cast();
                div = tce.getFirstChild().<DivElement>cast();
        ...
        for ( int iCol = 0; iCol < widget.getVisibleConditionColumnsCount(); iCol++ ) {
            DynamicColumn<BaseColumn> col = widget.getVisibleConditionColumn( iCol );
            ...
            int colSpan = 1;
            int width = col.getWidth();
            while ( iCol + colSpan < widget.getVisibleConditionColumnsCount() ) {
                DynamicColumn<BaseColumn> mergeCol = widget.getVisibleConditionColumn( iCol + colSpan );
            ...
            // Resize cell
            iCol = iCol + colSpan - 1;
            tce = widget.getHeaderRow( 2 ).getChild( iRow2ColColumn ).<TableCellElement>cast();
            div = tce.getFirstChild().<DivElement>cast();
            div.getStyle().setWidth( width,
                                     Unit.PX );
            iRow2ColColumn++;
        ...
        if ( multiRowColumnActionsOffset != -1 ) {
            ...
            if ( colOffsetIndex >= 0 ) {
                ...
                DynamicColumn<BaseColumn> col = widget.getVisibleColumn( resizeColumnIndex );
                tce = widget.getHeaderRow( 2 ).getChild( colOffsetIndex ).<TableCellElement>cast();
                div = tce.getFirstChild().<DivElement>cast();
                div.getStyle().setWidth( col.getWidth(),
                                         Unit.PX );
            }
        ...
        ColumnResizeEvent cre = new ColumnResizeEvent( widget.getVisibleColumn( resizeColumnIndex ),
                                                       resizeColumnWidth );
    ...
    private DivElement makeLabel(String text,
                                 int width,
                                 int height ) {
        ...
        switch ( iRow ) {
            case 0:
                ...
                for ( int iCol = 0; iCol < widget.getVisibleColumnsCount(); iCol++ ) {
                    ...
                    populateTableCellElement( modelCol,
                            width,
                            tce );
                    tce.setColSpan( colSpan );
                }
                break;

            case 1:
                // Splitter between "general" and "technical" condition details
                if ( widget.getVisibleConditionColumnsCount() > 0 || widget.getVisibleActionColumnsCount() > 0 ) {
                    tce = Document.get().createTDElement();
                    tce.setColSpan( widget.getVisibleConditionColumnsCount() + widget.getVisibleActionColumnsCount() );
                    tce.addClassName( resources.headerSplitter() );
                    tre.appendChild( tce );
                    widget.addSplitter( tce );
                }
                break;
            ...
            case 2:
                ...
                for ( int iCol = 0; iCol < widget.getVisibleConditionColumnsCount(); iCol++ ) {
                    ...
                    tce.appendChild( makeLabel( label.toString(),
                            width,
                            ( widget.isSiplitterCollapsed() ? 0 : resources.rowHeaderHeight() ) ) );
                    tce.setColSpan( colSpan );

                }

                //Action FactType
                for ( int iCol = 0; iCol < widget.getVisibleActionColumnsCount(); iCol++ ) {
                    DynamicColumn<BaseColumn> col = widget.getVisibleActionColumn( iCol );
            ...
            case 4:
                ...
                for ( DynamicColumn<BaseColumn> col : sortableColumns ) {
                    if ( col.isVisible() ) {
                        final HeaderSorter shp = new HeaderSorter( col, resources );
                        ...
                        tce.addClassName( resources.cellTableColumn( col.getModelColumn() ) );
                        tre.appendChild( tce );
                        widget.addSorter(shp, tce);
                    }
                }
                break;
        }

        widget.replaceHeaderRow(iRow, tre);
    }
